I've read the actual code across all three stages, the docs, the tests, and the Android parity file. I verified the plans' core claims against the source before judging. Here is the report.
...
**Coverage.** Both plans cover all three stages, name file + mechanism, rank by severity, and pair every fix with a label-free validation. Plan 2 is strictly more granular and finds more: it splits Plan 1's `H5` into `F1` (hip-origin semantics) and `F9` (coverage), splits `H2` into `F4` (regex) and `F5` (duplication), and adds two findings Plan 1 misses entirely — `F2` ("bent arms" can never fire because `_f(values.get("arms_straight_frac"), 1.0)` defaults a **missing** measurement to healthy; only `holds.py:_hold_metrics` ever produces that key, and the rep path never does) and `F6` (whole-clip percentile features include standing frames, so a pull-up with a still hand-low prefix can mis-bin to `muscle_up`, and a rest-dominant clip falls to `unknown`). These are the most valuable insights and Plan 1 does not have them.

**Correctness / feasibility.** Plan 2 is more precise but carries one arithmetic error in `F1`: it claims lockout fires "when the athlete's arm:torso ratio exceeds ~0.85". For a squat, `peak_height` = shoulder-above-hip ≈ 1.0 torso, so `lockout_pct = 100/arm`, and the fault fires when `100/arm < 85` → arm > 1.18, **not** 0.85. The conclusion (an anatomy-lottery false fault) is correct; the number is wrong. The `dead hang` claim ("fires on every squat unconditionally") is verified — `start_depth = -begin ≈ -1.0`, so `hang_pct ≈ -100/arm < 75` always. Plan 1's `H4` diagnosis (anchor travel in absolute image space is what rotation violates) is correct and simpler.

**Risk / edge-case handling.** Plan 2 is far stronger: four explicit self-critiques, a detailed Risks section, per-phase Rollback, Edge Cases, and Open Questions. The self-critique on Phase 4 roll-normalization is the single best piece of critical thinking in either plan — it admits the wrist-wrist "bar axis" assumption is false for rings/single-arm hangs, and that 0011's motion may be azimuth/pitch rather than roll, so the fix may not move the count. Plan 1's two self-critiques (relative-articulation may regress 0014; `R_true` calibration bias under-estimates azimuth) are also valuable, and Plan 1's mitigation for the view-gating calibration bias is better than Plan 2's.

**Test/validation completeness.** Plan 2 is more thorough: prefix-invariance, boundary-value (84.9/85.0/85.1), golden-payload, a text-level `Cues.kt` parity test, and per-fix acceptance criteria against `MAX_ACCEPTABLE_FPR=0.20`/`MIN_ACCEPTABLE_DETECTION=0.60`. Plan 1 contributes a genuinely useful **reword-invariance** test (mutate the `why` string, assert the fault set is unchanged) and a camera-motion null — both worth keeping.

**Clarity / actionability.** Plan 2's tasks each carry location/description/tokens/dependencies/steps/acceptance criteria; Plan 1 is more compact but less operational. Plan 2 is nonetheless very long (phase token budget sums to ~600k), and the verbosity is only partly value-bearing.

**Conciseness.** Plan 1 wins decisively. Plan 2's verbosity is largely justified (self-critique, edge cases), so I penalize only mildly.

## Missing Steps
- **Confidence relabeling (Plan 1 `L-1` / Task 4.3)** is absent from Plan 2. `classify.py:485-564` emits 0.70–0.98 "confidences" that are really margins-to-threshold; the `certain >= 0.65` gate reads them as probabilities. Cheap, small, worth adding.
- **View-gating calibration-bias mitigation** is weaker in Plan 2. It should adopt Plan 1's rule: gate PLANAR faults only when the view is *knowable* (`camera_side` agreement ≥ 0.70 and `R_true` inside `ANATOMICAL_PRIOR_RANGE`), else emit `unmeasured`; prefer the `declared_bin`/`view` from `sessions.csv` (`ingest.py:89`) over the estimator.
- **Threshold home.** Plan 2 proposes a new `barra/thresholds.py`; `docs/CORE.md:188` states "Thresholds are fixed in `config.py`". Putting them in `config.py` (Plan 1) honors the documented contract. If a new module is used, the doc contract must be amended, and the "fingerprinted before labels are seen" mechanism must move too.
- **Explicit non-re-litigation note for roll normalization.** The deviation-score normalisation deliberately keeps rotation (`test_rotation_is_not_removed`, `docs/CORE.md:147`). Plan 2's Phase 4 roll correction must be scoped to the **segmentation signal only**, never the normalised skeleton used for scoring, or it will look like it violates a documented decision.
- **Both plans ignore that the current test run is not green** (`Ran 107 tests`, 2 failures, 16 errors in this environment) and that the documented baseline is "137". Before a "behaviour-preserving" refactor gate (thresholds module, per-span fallback) can assert identical pre/post output, the baseline must be re-established in the correct `.venv` (mediapipe backend present) or the parity claim is unfalsifiable.

## Contradictions
- **Plan 2 F1 arithmetic:** "~0.85" vs the correct arm ratio of ~1.18 for the lockout lottery (conclusion unaffected).
- **Plan 2 internal ordering:** Phase 2 is numbered after Phase 1, but Task 1.1 notes "Phase 2 lands first in code order if sequencing demands" — a mild self-contradiction; the thresholds/structured-fields work should be sequenced first.
- **Plan 2 Task 1.2** is conditional ("measure `arms_straight_frac` ... if tokens allow; otherwise remove and document"), which is acceptable but leaves the outcome undecided.

## Improvements
- Fix Plan 2's `F1` lockout figure to ~1.18.
- Add Plan 1's **reword-invariance** test (the strongest single validation idea) and the confidence-margin relabeling.
- Add Plan 1's **view-gating "knowable" gate** to Plan 2's Phase 3.
- Put thresholds in `config.py` (matching `docs/CORE.md:188`), with a `Cues.kt` parity test as Plan 2 specifies.
- Keep Plan 2's per-span amplitude + per-span classification with single-span fallback and disagreement-refusal blocker; adopt Plan 1's lighter per-rep consistency check as a fallback for single-movement clips.
- Keep Plan 2's honest self-critique on roll normalization and the "0011 may be azimuth" caveat; make the segmentation-only scope explicit.
- Declare the `tecnicas-errores-comunes.md` items that keypoints cannot see (gaze, wrist loading, grip type, lumbar rounding, progression-skipping) in a `never_measured` list rather than silently omitting them.
